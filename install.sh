...
BOOTNODE2="/ip4/166.70.250.197/tcp/30333/p2p/12D3KooWNjyk4sAGCVeKk2qNABVzznJSLUKaKpTETzE8kfEzVKmd"
...
COMPOSE
  ...
  # Download WASM override (required for syncing past historical migration blocks)
  info "Downloading WASM runtime override..."
  mkdir -p "$OPERATOR_DIR/wasm-overrides"
  curl -sSL -o "$OPERATOR_DIR/wasm-overrides/materios_runtime.wasm" \
    "https://github.com/Flux-Point-Studios/materios/releases/download/override-sync/materios_runtime.wasm" || \
    warn "Failed to download WASM override. Node may have trouble syncing past migration blocks."
  ok "WASM override downloaded"
...
services:
  materios-node:
    ...
    volumes:
      ...
      - ./wasm-overrides:/wasm-overrides:ro
    ports:
