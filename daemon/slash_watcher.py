...
class ClassifierOutcome:
    ...
    NOT_FRAUD / TRANSIENT_L1_ERROR / OBSERVER_UNAVAILABLE are non-slash
    outcomes — the watcher moves on without dispatching. The remaining
    three are the SLASH outcomes and correspond directly to the
    pallet's FraudProof variants.
    """
    NOT_FRAUD = "not_fraud"
...
def classify_fraud(
    ...
    Returns:
        ...
        * ``None`` when (a) the evidence agrees with reality (honest
          ...
          (cannot conclude — defer to next tick). The caller
          distinguishes (a) and (b) via the observation's
          ``mismatches`` list.

    Priority order:
        1. ``TxNotFound`` (Kupo found nothing for cardano_tx_hash) —
           takes precedence over the field-mismatch variants because
           if the tx doesn't exist there's no concrete output to
           field-check.
        2. ``WrongBeneficiary`` (tx exists but no output pays the
           claimed beneficiary; we use a different
           candidate-payment-hash from the observer).
        3. ``WrongAmount`` (tx exists, pays the claimed beneficiary,
           but a different lovelace total).
        4. ``None`` (honest evidence).

    Safety contract: transient errors (Ogmios down, Kupo 500,
    chain-sync timeout) MUST NOT be classified as ``TxNotFound``. The
    observer emits ``kupo_no_matches_for_tx`` ONLY on a successful
    lookup that returned an empty match list — that's the truthful
    "tx is not on chain" signal. Any other mismatch tag means
    incomplete observation; the classifier returns ``None`` and the
    dispatcher defers.
    """
    # Slot 1: TxNotFound has highest priority. The settle attestor
    # uses the EXACT same `kupo_no_matches_for_tx` tag as its
    # tx-existence proof; we mirror that contract.
...
class CardanoSlashObserver(CardanoTxObserver):
    ...
    async def _first_payment_hash(
        ...
        """Return the FIRST output's 28-byte payment-key hash via Kupo.

        Returns ``None`` on any error (no output, decode failure, Kupo
        unreachable). The dispatcher treats None as "can't construct
        WrongBeneficiary proof" and defers the slash to the next tick
...
class SlashWatcher:
    ...
    async def _process_one_locked(
        ...
        #       This is the TxNotFound proof's truthful trigger.
        ...
        #       ogmios unreachable, malformed response). The observer
        #       surfaces non-`kupo_no_matches_for_tx` mismatch tags
        #       (`ogmios_tip_unavailable`, `kupo_match_missing_slot`,
        #       etc.) — we MUST NOT classify those as TxNotFound.
        if (
        ...
        proof = classify_fraud(request, observation)
        if proof is None:
